...
public final class Numbers {
    ...
    public static <T extends Number> double sum(T[] values) {
        int sum = 0;
        for (T value : values) sum += value.doubleValue();   // метод преобразования в double
   ...
   public static <T extends Number & Comparable<T>> T min(T a, T b) {
     ...
     */
     ...
     // возвращает Int
    public static Integer[] castIntToInteger(int[] values) {
        Integer[] arrInt = new Integer[values.length];
    ...
    // возвращает double
    public static Double[] castIntToDouble(int[] values) {
        Double[] arrInt = new Double[values.length];
        for (int i = 0; i < arrInt.length; i++) {
            arrInt[i] = (double) values[i];
        }
        return arrInt;
    }
}
